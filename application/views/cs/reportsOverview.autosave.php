      ...
      <div class="main-content">
        ...
        <!-- CONSIGNMENTS -->
            <div style="width: 90%;">
		        ...
		        <canvas id="canvas" width="1013" height="506" class="chartjs-render-monitor" style="display: block; width: 1013px; height: 506px; bac"></canvas>
		    ...
		    <?php
		    $january=$this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="1" AND sent_to_accounts="1" AND sent_to_cs="1" GROUP BY bol_cons_id');
		    $february=$this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="2" AND sent_to_accounts="1" GROUP BY bol_cons_id');
		    $march = $this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="3" AND sent_to_accounts="1" GROUP BY bol_cons_id');
		    $april = $this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="4" AND sent_to_accounts="1" GROUP BY bol_cons_id');
		    $may = $this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="5" AND sent_to_accounts="1" GROUP BY bol_cons_id');
		    $june = $this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="6" AND sent_to_accounts="1" GROUP BY bol_cons_id');
		    $july = $this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="7" AND sent_to_accounts="1" GROUP BY bol_cons_id');
		    $august=$this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="8" AND sent_to_accounts="1" GROUP BY bol_cons_id');
		    $september=$this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="9" AND sent_to_accounts="1" AND sent_to_cs="1" AND (cons_import_export="Import" OR cons_import_export="Export") GROUP BY bol_cons_id');
		    $october = $this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="10" AND sent_to_accounts="1" GROUP BY bol_cons_id');
		    $november = $this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="11" AND sent_to_accounts="1" GROUP BY bol_cons_id');
		    $december = $this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="12" AND sent_to_accounts="1" GROUP BY bol_cons_id');
		     ?>
		     <!-- //ALL CONSIGNMENTS DATA-->
		     <!-- ALL IMPORTS DATA -->
		    <?php
		    ...
		    $import9 = $this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="9" AND cons_import_export="Import" AND sent_to_accounts="1" AND sent_to_cs="1" GROUP BY bol_cons_id');
		    ...
		    $export9 = $this->db->query('SELECT cons_id FROM Consignment,cons_bill_of_landing where cons_id=bol_cons_id AND month(bol_added_on)="9" AND cons_import_export="Export" AND sent_to_accounts="1" AND sent_to_cs="1" GROUP BY bol_cons_id');
